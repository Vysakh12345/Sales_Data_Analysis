# 📊 Sales Data Analysis – Power BI Project

## 📌 Project Overview
This Power BI project provides a detailed analysis of sales performance across products, customers, promotions, and time periods.  
The dashboard highlights top-performing products, yearly trends, promotional impact, profitability, and customer purchasing behaviour.

The visuals are interactive and give a complete understanding of how sales, profit, and quantities fluctuate across multiple factors.

---

## 🧾 Dataset Columns
The dataset used for this dashboard includes the following fields:

- CustomerID  
- Order ID  
- Product ID  
- Date (dd/mm/yyyy)  
- Discount  
- Discount Percentage  
- Net Sales  
- Price Per Unit  
- Profit  
- PromotionID  
- Total Sales  
- Units Sold  


---

## 🚀 Key Metrics
According to the dashboard:

- **Total Sales:** 122M  
- **Total Profit:** 12.2M  
- **Total Quantity Sold:** 7.1K  
- **Total Orders:** 3510  

These KPIs summarise overall retail performance across 5 years (2020–2024).

---

## 🏆 Product Insights

### 🔹 Top 5 Products by Sales
- Apple iPhone 14 – 21M  
- Apple MacBook Air – 20M  
- Sony Bravia 55" TV – 19M  
- Samsung Galaxy S21 – 15M  
- HP Pavilion Laptop – 14M  

### 🔹 Bottom 5 Products by Sales
- Tupperware Lunch Box – 0.26M  
- L'Oreal Shampoo – 0.17M  
- Nivea Body Lotion – 0.08M  
- Dove Soap Pack – 0.08M  
- Colgate Toothpaste – 0.02M  

---

## 📦 Quantity Insights

### 🔹 Top 5 Products by Quantity Sold
- Apple iPhone 14 – 281  
- Raymond Suit – 274  
- Fossil Smartwatch – 269  
- Zara Casual Shirt – 269  
- IFB Microwave Oven – 259  

### 🔹 Bottom 5 Products by Quantity Sold
- Nivea Body Lotion – 219  
- Tupperware Lunch Box – 215  
- Milton Thermos Flask – 214  
- FabIndia Kurta – 210  
- Borosil Glass Set – 203  

---

## 💰 Profit Insights

### 🔹 Top 5 Products by Profit
- Apple iPhone 14 – 2.1M  
- Apple MacBook Air – 2.0M  
- Sony Bravia 55" TV – 1.9M  
- Samsung Galaxy S21 – 1.5M  
- HP Pavilion Laptop – 1.4M  

### 🔹 Bottom 5 Products by Profit
- Tupperware Lunch Box – 26K  
- L’Oreal Shampoo – 17K  
- Nivea Body Lotion – 8K  
- Dove Soap Pack – 8K  
- Colgate Toothpaste – 2K  

---

## 📈 Sales Trends
The “Sales Trends by Year” line graph shows:

- Sales peak consistently around **0.55M** in multiple periods  
- Spikes occur seasonally in **2020, 2021, 2022, 2023, and 2024**  
- Sales dip sharply during non-festive periods  
- Overall pattern indicates strong **year-end and promotional season activity**

---

## 🛍 Promotion Analysis
Average discount offered by promotion categories:

- Weekend Flash Sale – 23K  
- Clearance Sale – 18K  
- Summer Sale – 7K  
- New Year Special – 3K  
- Festive Diwali – 0K  

👉 Promotion types have a strong impact on volume, especially flash sales.

---

## 📊 Profit vs Net Sales
A scatterplot shows an almost perfect positive linear correlation:

- Higher Net Sales always corresponds to higher Profit  
- Indicates consistent pricing and discounting strategy  
- No major outliers or loss-making items

---

## 🏙 Sales by City
A map visual displays distribution across cities (data shown in interactive form in PBIX).

---

## 🎛 Filters Used in the Dashboard
- Date  
- Customer Name  
- Product Name  
- Promotion Name  

These allow users to dynamically explore individual transactions and trends.

---

## 📥 How to Use This Project
1. Download the `.pbix` file  
2. Open it using **Power BI Desktop**  
3. Use filters and visuals to explore insights  
4. Review detailed transactions in the table view  

---

## 🛠 Tools Used
- Power BI Desktop  
- DAX for calculated metrics  
- Power Query for transformations  
- Interactive dashboards & trend visuals  

---

